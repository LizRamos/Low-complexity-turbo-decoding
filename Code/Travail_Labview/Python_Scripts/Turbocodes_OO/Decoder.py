...
class Turbo_Decoder():
    ...
    def decode(self,received_message,sigma2):
        ...
        for iteration in range(self.nbIterations):
            ...
            decoded_message_iteration=[-99]+[int(lr) for lr in 0.5*(np.sign(LLR1)[1:]+1)]
            
            decoded_messages+=[decoded_message_iteration]
            ##############################################################################
...
#            LLR2=decoder2.compute_decision(received_message,LLR12perm,LLR21,
#                                          self.permutation,sigma2)

#            decoded_message2=[-99]+[int(lr) for lr in 0.5*(np.sign(LLR)[1:]+1)]
#            decoded_message2=Interleave(decoded_message2,self.reverse_permutation)
